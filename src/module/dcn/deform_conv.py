...
class DeformConv(nn.Module):
    ...
    def type_initialize(self, ori_type):
        if self.weight is not None:
            self.weight.data = self.weight.data.type(ori_type)
        self.initialize_type = True

    def forward(self, x, offset):
        if not self.initialize_type:
            self.type_initialize(x.type())
...
class ModulatedDeformConv(nn.Module):
    ...
    def type_initialize(self, ori_type):
        ...
        if self.weight is not None:
            self.weight.data = self.weight.data.type(ori_type)
        if self.bias is not None:
            self.bias.data = self.bias.data.type(ori_type)
        self.initialize_type = True
...
class ModulatedDeformConvPack(ModulatedDeformConv):
    def __init__(self, *args, extra_offset_mask=False, max_offset=30, **kwargs):
    ...
    def forward(self, x):
        ...
        offset = torch.clamp(torch.cat((o1, o2), dim=1), 0, self.max_offset)
