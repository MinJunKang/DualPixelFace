...
from src.model.stereodpnet.modules import feature_extraction, CostVolume, PSMNetHGAggregation, disp_regression
from src.model.stereodpnet.normal_module import ANM
...
class STEREODPNET(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        # Normal Branch (ANM)
        normal_results = self.normal_estimator([cost[0]], [cost_f[0]], batch) if self.option.model.predict_normal else None
        
        # Results
        cost_f = rearrange(cost_f, 'n b h w -> b n h w')
        cost_p = rearrange(cost_p, 'n b c h w -> b n c h w')
        normal = rearrange(normal_results[0], 'n b c h w -> b n c h w') if normal_results is not None else None
